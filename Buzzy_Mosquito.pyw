...
class assets:
    ...
    def animate(self,pos = None,size = None,rotate = 0,flipx = False,flipy = False,rate = None):
        ...
        if size == None:
            ...
            self.size = size
        frames = os.listdir(self.frame_folder)
        ...
        if self.rate_count == rate or self.for_once == True:
            self.rate_count = 0
            self.frame += 1
            self.image = pm.transform.rotate(pm.transform.scale(pm.image.load(f'{self.frame_folder}\\{frame}').convert_alpha(),(size[0],size[1])),rotate)
...
class multiple_asset:
    def __init__(self):
        self.ys = []
    def repeatperscreen(self,screenwidth,asset,frame_folder,size,pos,count,y_list = None,bias = 0,randomyrange = (0,0),moveby = 0,rotate = 0,rate = None,score = None):
        # print(asset)
        ...
        while len(asset) < count:
            if y_list == None:
                ...
                asset.append(assets(frame_folder,[pos[0] + len(asset) * gap,y_list[len(asset)] + bias],size))
        ...
        for object in asset:
            ...
            if object.pos[0] + object.size[0] < 0:
                if score != None:
                    score += 1
                if y_list == None:
                    object.pos = [gap * count - (object.size[0] + 5),random.randint(randomyrange[0],randomyrange[1])]
                else:
                    object.pos = [gap * count - (object.size[0] + 5),y_list[-1] + bias]
                self.ys.pop(0)
...
def main():
    ...
    while True:
        ...
        pipes2,score = pipes_2.repeatperscreen(width,pipes2,'obstacle',[100,height],pipe_pos2,4,moveby=pipes_velocity,y_list=pipe1y,bias = height + pipe_gap,score = score)
